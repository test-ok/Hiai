...
typedef enum {
    ...
    // return this error code when load xxx.caffemodel or xx.pb failed
    MODEL_CONTENT_ERROR = 25,
    // return this error code when parse xxx.prototxt or xx.param.txt failed
    MODEL_PARAMETER_ERROR = 26,
    // return this error code when not support this function
    NOT_SUPPORT_ERROR = 27,
    ALLOC_MEM_ERROR = 28,
...
// modify the type name from HIAI_DevPerf to HIAI_MixDevPerf
typedef enum {
...
// the TensorInfo which could get from npu model
typedef struct {
    int input_cnt;
    int output_cnt;
    int *input_shape;
...
// the TensorDescription which could get from npu model
typedef struct {
    int number;
    int channel;
    int height;
    int width;
    HIAI_MixDataType dataType; // option
...
// ModelManager
HIAI_MixModelManager *
HIAI_MixModelManager_Create(HIAI_MixModelListener *listener);
void HIAI_MixModelManager_Destroy(HIAI_MixModelManager *manager);

// ModelBuffer api
HIAI_MixModelBuffer *HIAI_MixModelBuffer_Create_From_File(const char *name,
                                                          const char *path,
                                                          HIAI_MixDevPerf perf,
                                                          bool mixflag);
HIAI_MixModelBuffer *
HIAI_MixModelBuffer_Create_From_Buffer(const char *name, void *modelbuf,
                                       uint32_t size, HIAI_MixDevPerf perf,
                                       bool mixflag);
...
// TensorBuffer api
HIAI_MixModelTensorInfo *
HIAI_MixModel_GetModelTensorInfo(HIAI_MixModelManager *manager,
...
HIAI_MixTensorDescription
HIAI_MixTensorBuffer_getTensorDesc(HIAI_MixTensorBuffer *b);
...
HIAI_MixMemBuffer *HIAI_MixMemBuffer_Create_From_Buffer(void *buffer,
                                                        const uint32_t size);
...
// CheckModelCompatibility api which used for mix model
bool HIAI_CheckMixModelCompatibility_From_File(HIAI_MixModelManager *manager,
                                               bool mixflag,
                                               const char *modelpath);
bool HIAI_CheckMixModelCompatibility_From_Buffer(HIAI_MixModelManager *manager,
                                                 bool mixflag, void *buffer,
                                                 const uint32_t size);

// build model on mobile api
// build model on mobile from file path
...
// build model on mobile from buffer
int HIAI_MixModel_BuildModel_FromBuffer(
    HIAI_MixModelManager *manager, HIAI_Mix_Framework frameworktype,
    const void *modelbuffer, unsigned int modelbuffersize,
    const void *modelparabuffer, unsigned int modelparabuffersize,
    const char *offlinemodelpath, bool mixflag);

// build model on mobile from memory
int HIAI_MixModel_BuildModel_ToMem(HIAI_MixModelManager *manager,
                                   ...
                                   unsigned int *outmodelsize, bool mixflag);

// load model api
int HIAI_MixModel_LoadFromModelBuffers(HIAI_MixModelManager *manager,
                                       HIAI_MixModelBuffer *bufferarray[],
                                       int nbuffers);

// run model api: the input of timeout is milliseconds
int HIAI_MixModel_RunModel(HIAI_MixModelManager *manager,
                           HIAI_MixTensorBuffer *inputs[], uint32_t niput,
                           HIAI_MixTensorBuffer *outputs[], uint32_t noutput,
                           uint32_t timeout, const char *modelname);
...
void HIAI_MixModel_SetBufferMultiple(HIAI_MixModelManager *manager,
                                     uint32_t multiple);
