...
bool checkModelCompatibility(HIAI_MixModelManager *mixModelManager,
                             const char *modelpath, bool isMixModel) {
    void *data = NULL;
    ...
    if (_fileExist(modelpath)) {
        ...
        if ((long)read_size != file_length) {
            ALOGE(" ERROR: read_size(%d) != fileLength(%d)", (int)read_size,
                  (int)file_length);
            free(data);
            data = NULL;
            fclose(fp);
        ...
        fclose(fp);
        checkresult = HIAI_CheckMixModelCompatibility_From_Buffer(
            mixModelManager, isMixModel, data, read_size);
        ALOGI(" HIAI_CheckMixModelCompatibility_From_Buffer "
              "checkresult : %d ",
              checkresult);
        if (data != NULL) {
            free(data);
...
Java_com_example_hiai_ModelManager_modelCompatibilityProcessFromBuffer(
    JNIEnv *env, jclass type, jbyteArray onlinemodelbuffer_,
    jbyteArray modelparabuffer_, jstring framework_, jstring offlinemodelpath_,
    jboolean isMixModel_) {
    jbyte *onlinemodelbuffer =
        env->GetByteArrayElements(onlinemodelbuffer_, NULL);
    ...
    std::string currentframe = std::string(framework);
    ...
    if (std::string(currentversion).compare("000.000.000.000") == 0) {
        result_code = NO_NPU;
    } else {
        bool checkresult = checkModelCompatibility(
            mixModelManager, offlinemodelpath, isMixModel_);

        if (checkresult) {
            result_code = CHECK_MODEL_COMPATIBILITY_SUCCESS;
        } else {
            ALOGI("start to HIAI_MixModel_BuildModel_FromBuffer.");
            int res = HIAI_MixModel_BuildModel_FromBuffer(
                mixModelManager, modelframework, onlinemodelbuffer,
                onlinemodelbuffersize, modelparabuffer, modelparabuffersize,
                offlinemodelpath, isMixModel_);

            ALOGI("HIAI_MixModel_BuildModel_FromBuffer "
                  "result_code: %d",
                  res);
            if (res != 0) {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_hiai_ModelManager_modelCompatibilityProcessFromFile(
    JNIEnv *env, jclass type, jstring onlinemodel_, jstring onlinemodepara_,
    jstring framework_, jstring offlinemodel_, jboolean isMixModel_) {
    ...
    if (std::string(currentversion).compare("000.000.000.000") == 0) {
        ...
        bool check = _fileExist(offlinemodel) &&
                     HIAI_CheckMixModelCompatibility_From_File(
                         mixModelManager, isMixModel_, offlinemodel);
        ALOGI("check result : %d", check);
        if (check) {
            result_code = CHECK_MODEL_COMPATIBILITY_SUCCESS;
        } else {
            int res = HIAI_MixModel_BuildModel_FromPath(
                mixModelManager, modelframework, onlinemodel, onlinemodepara,
                offlinemodel, isMixModel_);
            ALOGI("HIAI_MixModel_BuildModel_FromPath result_code : "
                  "%d",
                  res);
            if (res != 0) {
                result_code = BUILD_ONLINE_MODEL_FAILED;
